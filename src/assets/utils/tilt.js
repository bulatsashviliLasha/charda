const VanillaTilt = (function () {
  class t {
    ...
    destroy() {
      clearTimeout(this.transitionTimeout), this.updateCall !== null && cancelAnimationFrame(this.updateCall), this.reset(), this.removeEventListeners(), this.element.vanillaTilt = null, delete this.element.vanillaTilt, this.element = null;
    }

    onDeviceOrientation(t) {
      if (t.gamma === null || t.beta === null) return;
      this.updateElementPosition(), this.gyroscopeSamples > 0 && (this.lastgammazero = this.gammazero, this.lastbetazero = this.betazero, this.gammazero === null ? (this.gammazero = t.gamma, this.betazero = t.beta) : (this.gammazero = (t.gamma + this.lastgammazero) / 2, this.betazero = (t.beta + this.lastbetazero) / 2), this.gyroscopeSamples -= 1);
      const e = this.settings.gyroscopeMaxAngleX - this.settings.gyroscopeMinAngleX;
      const i = this.settings.gyroscopeMaxAngleY - this.settings.gyroscopeMinAngleY; const s = e / this.width;
      const n = i / this.height; const l = (t.gamma - (this.settings.gyroscopeMinAngleX + this.gammazero)) / s;
      const a = (t.beta - (this.settings.gyroscopeMinAngleY + this.betazero)) / n;
    ...
    onMouseMove(t) {
      this.updateCall !== null && cancelAnimationFrame(this.updateCall), this.event = t, this.updateCall = requestAnimationFrame(this.updateBind);
    }
    ...
    extendSettings(t) {
      ...
      for (const s in e) {
        if (s in t) i[s] = t[s]; else if (this.element.hasAttribute(`data-tilt-${s}`)) {
          const t = this.element.getAttribute(`data-tilt-${s}`);
          try {
            i[s] = JSON.parse(t);
          } catch (e) {
            i[s] = t;
